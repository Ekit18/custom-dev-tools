  ...
  return (
    <Card sx={{ minWidth: 275, mb: 2, position: 'relative' }}>
      <CardContent>
        ...
        <Box sx={{ mt: 2 }}>
          ...
          <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5, mb: 1 }}>
            ...
            {store.adminScopes.length > 5 && (
              <Chip label={`+${store.adminScopes.length - 5} more`} size='small' />
            )}
          </Box>
        </Box>
        ...
        <Box sx={{ mt: 1 }}>
          ...
          <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
            ...
            {store.storefrontScopes.length > 5 && (
              <Chip label={`+${store.storefrontScopes.length - 5} more`} size='small' color='secondary' />
            )}
          </Box>
        </Box>
        ...
        >
          <Typography variant='subtitle2' sx={{ mb: 1 }}>Store Tokens</Typography>
          <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
            ...
            <Tooltip title={copied === 'admin' ? 'Copied!' : 'Copy'}>
              <IconButton onClick={() => handleCopy(store.adminAccessToken, 'admin')}>
                <CopyAll fontSize='small' color={copied === 'admin' ? 'success' : 'inherit'} />
              </IconButton>
            </Tooltip>
          </Box>
          <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
            ...
            <Tooltip title={copied === 'storefront' ? 'Copied!' : 'Copy'}>
              <IconButton onClick={() => handleCopy(store.storefrontAccessToken, 'storefront')}>
                <CopyAll fontSize='small' color={copied === 'storefront' ? 'success' : 'inherit'} />
              </IconButton>
            </Tooltip>
          </Box>
          <Button
        ...
        >
          <Typography variant='subtitle2' sx={{ mb: 2, fontWeight: 600 }}>Store Tokens</Typography>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5, mb: 2 }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              <TextField
                ...
                size='small'
                fullWidth
                InputProps={{ readOnly: true, sx: { borderRadius: 2, fontSize: 13 } }}
              />
              <Tooltip title={copied === 'admin' ? 'Copied!' : 'Copy'}>
                <IconButton onClick={() => handleCopy(store.adminAccessToken, 'admin')} size='small' sx={{ ml: 0.5 }}>
                  <CopyAll fontSize='small' color={copied === 'admin' ? 'success' : 'inherit'} />
                </IconButton>
              </Tooltip>
            </Box>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              ...
              />
              <Tooltip title={copied === 'storefront' ? 'Copied!' : 'Copy'}>
                <IconButton onClick={() => handleCopy(store.storefrontAccessToken, 'storefront')} size='small' sx={{ ml: 0.5 }}>
                  <CopyAll fontSize='small' color={copied === 'storefront' ? 'success' : 'inherit'} />
                </IconButton>
              </Tooltip>
            </Box>
